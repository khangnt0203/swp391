...
const { TabPane } = Tabs;
export default function DetailFreelancerReport() {
  ...
  const column = [
...

    {
      ...
      dataIndex: "content",
      render: (e,item)=>{
        return <div style={{fontStyle:'italic'}}>{item.content}</div>
      }
    },
    {
  ...
  function loadReport() {
    let map = new Map();
    getAuth(
      `/Report/Account?isReporter=false&take=${take}&skip=${skip}&status=${status}&roleAdminFilter=1`
    ).then((response) => {
  ...
  function loadReported() {
    let map = new Map();
    getAuth(
      `/Report/Account?isReporter=${isReporter}&take=${take}&skip=${skip}&roleAdminFilter=1&status=0`
    ).then((response) => {
      if (response.data.code === 1) {
        response.data.data.map((r) => {
          map.set(r.id, r);
    ...
    getAuth(
      `/Report/Account?isReporter=${isReporter}&take=${take}&skip=${skip}&roleAdminFilter=1&status=1`
    ).then((response) => {
      if (response.data.code === 1) {
        response.data.data.map((r) => {
          map.set(r.id, r);
    ...
    getAuth(
      `/Report/Account?isReporter=${isReporter}&take=${take}&skip=${skip}&roleAdminFilter=1&status=2`
    ).then((response) => {
      if (response.data.code === 1) {
        response.data.data.map((r) => {
          map.set(r.id, r);
    ...
    getAuth(
      `/Report/Account?isReporter=${isReporter}&take=${take}&skip=${skip}&roleAdminFilter=1&status=3`
    ).then((response) => {
      if (response.data.code === 1) {
        response.data.data.map((r) => {
          map.set(r.id, r);
