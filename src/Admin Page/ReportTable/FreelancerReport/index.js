...
const { TabPane } = Tabs;
export default function FreelancerReport() {
  ...
  function loadReport() {
    let map = new Map();
    getAuth(
      `/Report/Account?isReporter=true&take=${take}&skip=${skip}&status=${status}&roleAdminFilter=1`
    ).then((response) => {
      ...
      >
        <Radio.Group value={solution} onChange={onChangeSolution}>
          <Space direction="vertical">
            <Radio value={0}>Payment freelancer and deduction client</Radio>
